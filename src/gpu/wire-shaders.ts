 ...
 * 회전은 매듭 자신의 축(Z) 자전 → 고정 기울기(X) → 커서 요(Y) 순서라
 * 자전 중에도 옆으로 납작해지지 않습니다.
...
const DIST: f32 = 3.2;   // 카메라 거리 (반지름 1 기준)
...
fn vs_main(@builtin(vertex_index) vi: u32) -> VOut {
  ...
  let cz = cos(u.angleZ);
  let sz = sin(u.angleZ);
  p = vec3f(p.x * cz - p.y * sz, p.x * sz + p.y * cz, p.z);
...
fn fs_main(in: VOut) -> @location(0) vec4f {
  ...
  let alpha = mix(0.06, 0.60, in.shade) * u.fade;
